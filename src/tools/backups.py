    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            if isinstance(response, list):
                backup_data = response[0] if response else {}
            else:
                _raw = response.get("data", response)
                backup_data = _raw[0] if isinstance(_raw, list) else _raw
...
async def get_backup_details(
    ...
    for backup in backups:
        if backup["filename"] == backup_filename:
            logger.info(
                sanitize_log_message(
                    f"Retrieved details for backup '{backup_filename}' in site '{site_id}'"
                )
            )
            return backup
...
async def delete_backup(
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            logger.info(
                sanitize_log_message(
                    f"Successfully deleted backup '{backup_filename}' from site '{site_id}'"
                )
            )
            log_audit(
                operation="delete_backup",
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            if create_pre_restore_backup:
                ...
                logger.info(
                    sanitize_log_message(f"Pre-restore backup created: {pre_restore_backup_id}")
                )
        ...
        logger.error(
            sanitize_log_message(f"Failed to restore from backup '{backup_filename}': {e}")
        )
        log_audit(
            operation="restore_backup",
            parameters=parameters,
...
async def get_backup_status(
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            status_data = await client.get_backup_status(site_id=site_id, operation_id=operation_id)
...
async def get_restore_status(
    ...
    try:
        ...
        logger.error(
            sanitize_log_message(f"Failed to get restore status for '{operation_id}': {e}")
        )
...
async def schedule_backups(
    ...
    try:
        ...
        logger.error(
            sanitize_log_message(f"Failed to configure backup schedule for site '{site_id}': {e}")
        )
        log_audit(
            operation="schedule_backups",
            parameters=parameters,
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            if not schedule_data:
                logger.info(
                    sanitize_log_message(f"No backup schedule configured for site '{site_id}'")
                )
                return {
                    "configured": False,
        ...
        logger.error(
            sanitize_log_message(f"Failed to get backup schedule for site '{site_id}': {e}")
        )
        raise
